...
from home.models import Profile, User, Project, Message, Coding, Descr
...
def home(request):
    ...
    homedesc=Descr.objects.all()
    context = {'posts': posts, 'projects': projects, 'homedetail':homedesc}
...
def coding(request):
    send_message(request)
    codings = Coding.objects.order_by('-sno').all()
    context = {'codings': codings}
